...
router.get('/', authMiddleware, async (req: Request, res: Response) => {
  try {
    const sessoes = await db.query<any>(
      `SELECT s.id, s.usuario_id, s.token_hash, s.ip_address,
...
router.get('/ativas', authMiddleware, async (req: Request, res: Response) => {
  try {
    const sessoes = await db.query<any>(
...
router.get('/count', authMiddleware, async (req: Request, res: Response) => {
  try {
    const result = await db.query<{ count: string }>(
      `SELECT COUNT(*) as count FROM sessoes WHERE expires_at > NOW()`
    );
...
router.get('/usuario/:usuario_id', authMiddleware, async (req: Request, res: Response) => {
  try {
    const { usuario_id } = req.params;
    const sessoes = await db.query<any>(
      `SELECT s.id, s.ip_address, s.user_agent, s.expires_at,
...
router.get('/:id/detalhe', authMiddleware, async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    ...
    const [sessoes, historico] = await Promise.all([
      ...
      db.query<any>(
        `SELECT h.id, h.sessao_id, h.acao, h.descricao, h.created_at
...
router.get('/:id/historico', authMiddleware, async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const historico = await db.query<any>(
      `SELECT h.id, h.sessao_id, h.acao, h.descricao, h.created_at
       FROM historico_sessoes h
       WHERE h.sessao_id = $1
       ORDER BY h.created_at DESC`,
...
router.post('/:id/forcar-logout', authMiddleware, async (req: Request, res: Response) => {
  try {
    const { id } = req.params;

    // Verificar se a sessao existe
    const sessoes = await db.query<any>(
      'SELECT id FROM sessoes WHERE id = $1',
      [id]
    );
...
router.post('/:id/refresh', authMiddleware, async (req: Request, res: Response) => {
  try {
    ...
    const sessoes = await db.query<any>(
      'SELECT id, refresh_token, expires_at FROM sessoes WHERE id = $1',
      [id]
    );
    ...
    await db.query(
      'UPDATE sessoes SET expires_at = $1, ultima_atividade = NOW() WHERE id = $2',
      [novaExpiracao.toISOString(), id]
    );
